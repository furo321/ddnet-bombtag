...
{
	if(((CGameControllerBomb *)(m_pGameServer->m_pController))->m_pLoadBestTimeResult)
		return; // already in progress

	auto LoadBestTimeResult = std::make_shared<CScoreLoadBestTimeResult>();
	((CGameControllerBomb *)(m_pGameServer->m_pController))->m_pLoadBestTimeResult = LoadBestTimeResult;
...
void CScore::SaveTeam(int ClientID, const char *pCode, const char *pServer)
{
	if(RateLimitPlayer(ClientID))
		return;
	auto *pController = ((CGameControllerBomb *)(GameServer()->m_pController));
