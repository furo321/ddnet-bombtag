...
void CGameContext::OnChangeInfoNetMessage(const CNetMsg_Cl_ChangeInfo *pMsg, int ClientId)
{
	...
	// Bombtag
	m_pController->OnSkinChange(pMsg->m_pSkin, pMsg->m_UseCustomColor, pMsg->m_ColorBody, pMsg->m_ColorFeet, ClientId);
...
void CGameContext::OnInit(const void *pPersistentData)
{
	...
	else if(!str_comp(Config()->m_SvGametype, "bomb"))
